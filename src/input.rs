...
pub fn read_store_data_files(
    config: &mut config::AppConfiguration,
    fields: HashMap<PathBuf, HashSet<String>>,
) -> ResultApp<(rusqlite::Connection, bool)> {
    let mut fi = Vec::new();
...
fn select_storage_loc(
    fi: &Vec<fs::File>,
    config: &config::AppConfiguration,
) -> ResultApp<(&'static str, bool)> {
    ...
    let total_memory_usage: usize = fi
        .iter()
        .map(|file| {
            file.metadata()
                .expect("No Metadata Was Found: Size of File is needed")
                .len() as usize
        })
        .sum();
    ...
    if !config.debug_mode() && config.can_be_in_memory_db(total_memory_usage) {
        info!(
            "All the files is estimated to requiere {} MB, TMP Database will be created in memory",
            total_memory_usage
        );
        ...
        if PathBuf::from("./rossete-tmp").exists() {
            crate::warning!(
                "Previous TMP Storage DB was Found, Proceeding to replace it with new one"
            );
    ...
    loop {
        if let Ok(query) = data_rx.recv_timeout(std::time::Duration::from_millis(150)) {
            if query.len() == 0 {
                ...
                // THIS CORRESPOND TO THE ID OF THE FILE IN NUMERIC FORM
...
fn create_tables(
    con: mpsc::Sender<String>,
    files: &HashMap<PathBuf, config::FileSpecs>,
    input_fields: &HashMap<PathBuf, HashSet<String>>,
) -> ResultApp<()> {
    for file in files.keys() {
        let file_type = files[file].get_file_type();
        if file_type.is_json() || file_type.is_xml() {
            let campos = input_fields[file]
                .iter()
                .map(|camp| camp.clone())
                .collect::<Vec<_>>();
            ...
            for iter in iteradores.keys() {
                ...
                info!(
                    "The following table was created in the database: {}",
                    &table_name
                );
                ...
                query.extend(
                    format!(
                        "CREATE TABLE {} (\"col_id\" INTEGER PRIMARY KEY AUTOINCREMENT, \"",
                        table_name
                    )
                    .chars(),
                );
...
fn get_table_name_with_iterator(
    path: &PathBuf,
    ...
    loop {
        if threads.len() < config.get_reading_theads() && current_file < files.len() {
            ...
            let hand = std::thread::spawn(move || -> ResultApp<()> {
                ...
                } else if file_type.is_tsv() {
                    // It is the same but it uses tabs
                    read_csv(new_id, path, specs, conn, rc, fields)
...
fn read_csv(
    ...
    for row in csv_file.deserialize() {
        let row_data: HashMap<String, String> = match row {
            ...
            Err(error) => {
                error!(
                    "CSV Reader could not extract row from file {}",
                    path.display()
                );
        ...
        let data = fields
            ...
            .map(|column| {
                row_data
                    .get(column)
                    .expect("There is a missing value in a CSV row")
                    .clone()
            })
            .collect::<Vec<_>>();
        let mut query_buffer =
            String::with_capacity(fields.iter().map(|f| f.len()).sum::<usize>() + 100);
