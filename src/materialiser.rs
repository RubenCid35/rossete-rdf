...
use crate::mappings::{maps::Mapping, parts::Parts};
use crate::ResultApp;
use crate::{error, info, warning}; // Debug and Message Print
...
pub fn rdf_procedure(
    ...
    let file_thread = thread::spawn(move || write_file(config_arc2, file_rx, num_maps));
...
fn create_rdf(
    file_con: mpsc::Sender<Vec<u8>>,
    db: Arc<Mutex<rusqlite::Connection>>,
    mappings: Vec<Mapping>,
    config: Arc<config::AppConfiguration>,
) -> ResultApp<()> {
    let max_threads = config.get_writing_theads();
    let mut threads: Vec<thread::JoinHandle<Result<(), ApplicationErrors>>> =
        ...
        if threads.len() < max_threads && current_map < mappings.len() {
            ...
            let handler = thread::spawn(move || -> ResultApp<()> {
                if output_format.is_nt() {
                    ...
                    warning!(
                        "The output format of the result cann't be processed. FORMAT: {:?}",
                        &output_format
                    );
                    rc.send(current_map)?;
                    write.send(Vec::new())?;
            ...
            let rc = rc_rx.recv()?;
            let thread_id = threads_id
                .iter()
                .position(|x| x == &rc)
                .expect("Thread ID was not found");
...
fn create_rdf_nt<'a>(
    id: usize,
    map: Mapping,
    rc: mpsc::Sender<usize>,
    db: Arc<Mutex<rusqlite::Connection>>,
    write: mpsc::Sender<Vec<u8>>,
    tables: Arc<HashMap<String, (String, String, Parts)>>,
) -> ResultApp<()> {
    let table_name = map.get_table_name()?;
    info!(
        "RDF FROM DB TABLE: {:<30} AND MAP: {}",
        &table_name,
        map.get_identifier()
    );
    ...
    let (rows, id_col) = match select_main_columns(&table_name, main_columns, &db) {
        ...
        Err(error) => {
            error!(
                "Something Ocurred while selecting the main columns from the table {}.",
                &table_name
            );
            rc.send(id)?;
            write.send(Vec::new())?;
    ...
    let (temp, input) = if let Parts::Template {
        template,
        input_fields,
    } = subject_map.get_template().unwrap()
    {
...
fn generete_join_statements(
    map: &Mapping,
    predicates: &Vec<&Parts>,
    table_name: &String,
    tables: &Arc<HashMap<String, (String, String, Parts)>>,
) -> HashMap<usize, (bool, String, String, Vec<String>)> {
    predicates
        .iter()
        .enumerate()
        .filter(|(_, obj)| obj.is_parent())
        .map(|(i, &obj)| {
            if let Parts::PredicateObjectMap {
                predicate: _,
                object_map,
            } = obj
            {
                let other_map = object_map
                    .iter()
                    .filter(|obj| obj.is_parent())
                    .map(|obj| {
                ...
                let query_table = generate_join_template_query(
                    &table_name,
                    &other_map,
                    &iter,
                    object_map,
...
fn select_main_columns(
    table_name: &String,
    main_columns: HashSet<String>,
    db: &Arc<Mutex<rusqlite::Connection>>,
) -> ResultApp<(Vec<Vec<String>>, HashMap<String, usize>)> {
...
fn format_uri(mut url: String, input: &Vec<String>) -> String {
    ...
    if url.contains(' ') {
        warning!(
            "INVALID URI. The folowing URI Contains Spaces so it will be discarded. URI: {}",
            url
        );
        return String::new();
    }
...
fn rdf_term(
    map: &Mapping,
    predicate_part: &Parts,
    from_table: &Vec<String>,
...
fn term_from_join_object(
    map: &Mapping,
    pre: &Parts,
    db: Arc<Mutex<rusqlite::Connection>>,
    from_table: &Vec<String>,
    columns: &HashMap<String, usize>,
    join_data: &(bool, String, String, Vec<String>),
    warn: &mut bool,
) -> ResultApp<String> {
    ...
    let row = match row {
        ...
        Err(error) => match error {
            rusqlite::Error::QueryReturnedNoRows => return Ok(String::new()),
...
fn generate_join_template_query(
    table_name: &String,
    other_map: &String,
    map_iterator: &String,
    objects: &Vec<Parts>,
    ...
    let (other_template, input) = if let Parts::Template {
        template,
        input_fields,
    } = other_temp
    {
...
fn get_predicate(predicate: &String, map: &Mapping, tags: bool, warn: &mut bool) -> String {
    ...
    if let Some(url) = parts.next() {
        ...
        if let Some(pre_url) = prefixes.get(&prefix) {
            ...
            if *warn {
                warning!(
                    "There is a missing prefix definition \"{}\" for the following map: {}",
                    prefix,
                    map.get_identifier()
                );
                *warn = false;
