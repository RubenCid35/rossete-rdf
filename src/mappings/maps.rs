...
impl Mapping {
    ...
    pub fn is_valid(&self) -> crate::ResultApp<()> {
        ...
        for component in &self.components {
            if component.is_predicate() {
                ...
                crate::error!(
                    "Invalid term appears at the wrong level in  the mapping: {}",
                    self.identificador
                );
    ...
    pub fn source_file(&self) -> ResultApp<&path::PathBuf> {
        let source = self.components.iter().find_map(|comp| match comp {
            parts::Parts::LogicalSource { source, .. } => Some(source),
            _ => None,
        });
    ...
    pub fn get_join_fields(&self) -> ResultApp<Vec<(String, std::collections::HashSet<String>)>> {
        ...
        for pre in self.get_predicates().iter() {
            ...
            if pre.is_join() {
                if let parts::Parts::PredicateObjectMap {
                    predicate: _,
                    object_map,
                } = pre
                {
    ...
    pub fn get_subject(&self) -> &parts::Parts {
        self.components
            .iter()
            .filter(|comp| comp.is_subjectmap())
            .nth(0)
            .unwrap()
    }
    ...
    pub fn get_table_name(&self) -> ResultApp<String> {
        if let Some(parts::Parts::LogicalSource {
            source,
            reference_formulation,
            iterator,
        }) = self.components.iter().find(|&p| p.is_logicalsource())
        {
            if !iterator.is_empty() {
                Ok(format!(
                    "\"db-{}-{:?}-{}\"",
