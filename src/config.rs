...
use crate::ResultApp;
use crate::{error, info, time_info, warning};
...
impl std::fmt::Debug for AppConfiguration {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        writeln!(
            f,
            "Note: Some Information is useless as its delimiter and header in no CSV Files"
        )?;
...
impl AppConfiguration {
    pub fn new(output_path: PathBuf) -> Self {
        let output_format =
            OutputFormat::from_str(output_path.extension().unwrap().to_str().unwrap_or("nt"));
    ...
    pub fn remove_unused_files(&mut self, files: Vec<(PathBuf, AcceptedType)>) {
        ...
        files
            ...
            .for_each(|(f, t)| self.add_data_file(f.clone(), t.clone()))
    }
        ...
        if !self.file_specs.contains_key(&path) {
            ...
            match file_type {
                AcceptedType::Unspecify => {
                    file_type2 = AcceptedType::from_str(
                        &path
                            .extension()
                            .expect("File with No extension")
                            .to_str()
                            .unwrap()
                            .to_lowercase(),
                    )
    ...
    pub fn parse_file_specs(
        json_data: &serde_json::Value,
    ) -> ResultApp<HashMap<PathBuf, FileSpecs>> {
        ...
        if let Some(files) = json_data.get("files-data") {
            ...
            for f in files.as_array().unwrap() {
                ...
                if let Some(d) = f.get("encoding") {
                    current_spec
                        .set_encoding(get_encoding_from_str(&d.as_str().unwrap().to_uppercase()));
                }
                // TYPE
                if let Some(d) = f.get("file-type") {
                    current_spec
                        .set_file_type(AcceptedType::from_str(&d.as_str().unwrap().to_lowercase()));
                }
                // Delimiter
    ...
    match config_path {
        Some(path) => {
            let mut config_file = match std::fs::File::open(path) {
                ...
                Err(error) => {
                    error!(
                        "Error Trying Opening Configuration File: {:?}",
                        ApplicationErrors::from(error)
                    );
            ...
            if let Err(error) = config_file.read_to_string(&mut json_tmp) {
                error!(
                    "Error Trying Reading Configuration File: {:?}",
                    ApplicationErrors::from(error)
                );
                warning!("Given the error, it will be used the default configuration to proceed with the parsing");
                return AppConfiguration::new(output_file.clone());
            ...
            match AppConfiguration::from_json(output_file.clone(), json_config) {
                ...
                Err(error) => {
                    error!(
                        "Error parsing the Json Values to the configuration: {:?}",
                        error
                    );
                    warning!("Given the error, it will be used the default configuration to proceed with the parsing");
        ...
        None => AppConfiguration::new(output_file.clone()),
    }
}
/*
